...
func NewModelRegistry(pool *PoolManager) *ModelRegistry {
	return &ModelRegistry{Pool: pool, index: map[string]map[string]bool{}}
}
...
func (r *ModelRegistry) Refresh(ctx context.Context) {
	...
	for _, account := range accounts {
		...
		for _, spec := range specs {
			...
			if next[spec.ID] == nil {
				next[spec.ID] = map[string]bool{}
			}
			next[spec.ID][account.ID()] = true
		}
	...
	if account != nil && account.Enabled {
		...
		for _, spec := range specs {
			...
			if r.index[spec.ID] == nil {
				r.index[spec.ID] = map[string]bool{}
			}
			r.index[spec.ID][id] = true
		}
		r.mu.Unlock()
		account.Models = models
	} else if account != nil {
